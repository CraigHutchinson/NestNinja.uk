---
layout: page
title: Community
permalink: /community/
---

Join the NestNinja community! Whether you're building your first bird box or contributing code, we'd love to have you.

---

## 💬 Get Involved

### GitHub Discussions

The best place for technical discussions, feature requests, and general questions.

<div class="cta-buttons" style="justify-content: flex-start;">
  <a href="https://github.com/CraigHutchinson/NestNinja.open/discussions" class="btn btn-primary">Join GitHub Discussions →</a>
</div>

Topics:
- 💡 Ideas and feature requests
- 🔧 Build help and troubleshooting
- 📸 Share your bird box photos and videos
- 🐛 Bug reports and issues
- 📚 Documentation improvements

### GitHub Repository

NestNinja follows an **open core model**: the full community edition firmware is open source, while premium features are developed separately.

<div class="cta-buttons" style="justify-content: flex-start;">
  <a href="https://github.com/CraigHutchinson/NestNinja.open" class="btn btn-secondary">View on GitHub →</a>
</div>

**Community Edition (Open Source):**
- ✅ Full bird box monitoring capabilities
- ✅ Local video recording and streaming
- ✅ Motion detection and alerts
- ✅ Web-based configuration interface
- ✅ WiFi setup and network management
- ✅ Complete ESP32-P4 firmware source code

**Premium Features (Commercial Products):**
- ⭐ Advanced AI species recognition
- ⭐ Cloud backup and remote access
- ⭐ Multi-box fleet management
- ⭐ Professional API access
- ⭐ Priority support and updates

*Note: Repository is currently private during initial development. We'll announce when it goes public!*

---

## 🌐 NestNinja Hub

**[NestNinja Hub]({{ site.hub_url }})** is the community camera feed discovery site. Once your NestNinja device is set up, you'll be able to opt in to listing your live feed so other bird watchers can find and watch it.

<div class="cta-buttons" style="justify-content: flex-start;">
  <a href="{{ site.hub_url }}" class="btn btn-primary">Visit NestNinja Hub →</a>
</div>

- Browse live nest cameras from across the UK
- Filter by location or species
- Opt in or out any time — your feed, your choice
- No account needed to browse

*Hub is in the planning stage, launching after the NestNinja firmware reaches stable release (late 2026 target).*

---

## 📱 Social Media

**Twitter:** [@NestNinjaUK](https://twitter.com/NestNinjaUK)
- Follow for project updates
- See community builds
- Share your bird watching adventures

---

## 🤝 Contributing

### Support Tiers

**Premium Product Customers:**
- Direct email support
- Priority bug fixes
- Feature requests considered
- Professional documentation

**Smart Camera Module Users:**
- Community forum support
- User guides and tutorials
- Active community help

**DIY Component Builders:**
- Community forum support only
- Full open-source Community Edition firmware
- Community-contributed guides
- Self-service troubleshooting

We welcome all kinds of contributions to the open source Community Edition:

### Future Code Contributions
- Fix bugs
- Add features
- Improve documentation
- Optimize performance
- Write tests

### Hardware Contributions
- Test different cameras
- Design enclosures
- Create PCBs
- Improve power efficiency

### Documentation
- Write tutorials
- Create diagrams
- Record videos
- Translate content

### Community Support
- Answer questions
- Review pull requests
- Share your builds
- Help newcomers

**How to Contribute:**
1. Check [open issues](https://github.com/CraigHutchinson/NestNinja.open/issues)
2. Fork the repository
3. Create a feature branch
4. Make your changes
5. Submit a pull request

---

## 🎯 Community Projects

### Project Showcase

Share your NestNinja build! We love to see:
- Custom enclosures and installations
- Creative mounting solutions
- Solar power setups
- Multi-box installations
- Interesting bird footage

**Submit your project:** Open a discussion on GitHub with photos, videos, and details!

### Featured Builds

(Coming soon - community builds will be featured here)

---

## 📧 Contact

### General Inquiries
Email: [hello@nestninja.uk](mailto:hello@nestninja.uk)

### Technical Support
Please use [GitHub Issues](https://github.com/CraigHutchinson/NestNinja.open/issues) for technical support so the entire community can benefit from the discussion.

### Media & Press
Email: [press@nestninja.uk](mailto:press@nestninja.uk)

---

## 📜 Code of Conduct

We're committed to providing a welcoming and inclusive environment. Please:

- ✅ Be respectful and considerate
- ✅ Welcome newcomers and help them learn
- ✅ Focus on constructive feedback
- ✅ Respect different viewpoints and experiences
- ❌ No harassment, trolling, or discrimination
- ❌ No spam or off-topic content

Violations may result in removal from the community.

---

## 🎓 Resources

### Learning Materials

**ESP32 Development:**
- [ESP-IDF Programming Guide](https://docs.espressif.com/projects/esp-idf/en/latest/)
- [ESP32-P4 Documentation](https://files.waveshare.com/wiki/common/Esp32-p4_technical_reference_manual_en.pdf)

**Bird Watching:**
- [RSPB Bird Identifier](https://www.rspb.org.uk/birds-and-wildlife/wildlife-guides/identify-a-bird/)
- [BTO Nest Recording Scheme](https://www.bto.org/our-science/projects/nest-record-scheme)

**DIY Bird Boxes:**
- [RSPB Nestbox Guide](https://www.rspb.org.uk/birds-and-wildlife/advice/how-you-can-help-birds/nestboxes/)
- Best practices for bird-friendly designs

---

## 🏆 Contributors

Thank you to everyone who has contributed to NestNinja!

(GitHub Contributors widget will be added here)

---

## 📰 Newsletter

(Coming soon - Sign up for project updates and bird watching tips)

---

## 🌟 Support the Project

The NestNinja Community Edition is free and open source. If you find it useful, consider:

- ⭐ **Starring** the [GitHub repository](https://github.com/CraigHutchinson/NestNinja.open)
- 🐦 **Sharing** about NestNinja on social media
- 📝 **Writing** about your experience
- 🤝 **Contributing** code or documentation
- 🛒 **Purchasing** a premium product to support development
- ☕ **Buying us a coffee** (coming soon - if we set up sponsorship)

---

<div class="cta-buttons">
  <a href="/documentation" class="btn btn-secondary">← Documentation</a>
  <a href="/about" class="btn btn-primary">About →</a>
</div>
